...
void SettingsDialog::updateSettings()
{
    ...
    // Insert Port Settings into table

    QSqlTableModel *PortTable;
    QSqlQuery Query(mydb);
    QSqlError Err;

      // Open Port Table and updatesetting
     PortTable = new QSqlTableModel(0, mydb);
     PortTable->setTable("Port");
     PortTable->select();
     Query.prepare("TRUNCATE TABLE Port");
     Query.exec();
     Query.prepare("INSERT INTO Port (Name, Baudrate, Databits, Parity, Stopbits, Flowcontrol) "
                       "VALUES (:Name, :Baudrate, :Databits, :Parity, :Stopbits, :Flowcontrol)");
     Query.bindValue(":Name", currentSettings.name);
     Query.bindValue(":Baudrate", currentSettings.baudRate);
     Query.bindValue(":Databits", currentSettings.dataBits);
     Query.bindValue(":Parity", currentSettings.parity);
     Query.bindValue(":Stopbits", currentSettings.stopBits);
     Query.bindValue(":Flowcontrol", currentSettings.flowControl);
     if (!Query.exec())
